...
SELECT	nwea_fall.school_code
		...
		(nullif(nwea_fall.adv_n+nwea_fall.acc_n+nwea_fall.prf_n+nwea_fall.bsc_n+nwea_fall.lmt_n, 0))*100 , 1)
												AS		prof_lep_perc
...
/*
	   Name:	prof_hs_est_lep_legacy_v
	   Author:	Damico, Nicholas J
	   Purpose:	Query that estimates the percentage of Limited English Proficiency (LEP) students 
	   proficient or better according to legacy state testing data (OAA, OGT) by subject and year 
	   for all students and in a school. NWEA data is used to make the proficiency estimates. 
	   Estimates are for ELA and Math ONLY. 
	   Change Log:
	   Date		        Who				       What
	   *************   **********************  ****************
