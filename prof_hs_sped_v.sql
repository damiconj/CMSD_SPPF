...
SELECT	perf_sped_n.school_code
		,perf_sped_n.school_year
		,perf_sped_n.test_subject
		,round((perf_sped_n.adv_n +perf_sped_n.acc_n + perf_sped_n.prf_n) / 
		(nullif(perf_sped_n.adv_n+perf_sped_n.acc_n+perf_sped_n.prf_n+perf_sped_n.bsc_n+perf_sped_n.lmt_n, 0))*100 , 1)
												AS		prof_perc
		,perf_sped_n.adv_n+perf_sped_n.acc_n+perf_sped_n.prf_n+perf_sped_n.bsc_n+perf_sped_n.lmt_n
												AS		sped_n

/*
	   Name:	prof_hs_sped_v
	   Author:	Damico, Nicholas J
	   Purpose:	Query that calcualtes the percentage of Special Education (SPED) students proficient or higher by 
	   subject and year for all students in a school, based on common core state testing data (PARCC, AIR). Student 
	   totals for the denominator are based on students in testing table and potentially miss untested students that 
	   have no records there. PROF_LEVEL_STATE_AUDIT has official state data that can be used to calculate total 
